...
export default function Register() {
  ...
  const handleGroupChange = (e) => {
    ...
    let filteredValue =
      name === "email" ? filterEmailChars(value) : filterAllSpecialChars(value);

    if (
      name === "noTelp" &&
      filteredValue !== "" &&
      !/^\d+$/.test(filteredValue)
    ) {
      return;
    }
        ...
        allTeamsData.forEach((team, teamIdx) => {
          team.members.forEach((member, memberIdx) => {
            if (member.pasFotoFile)
              formData.append(
                `t${teamIdx}_m${memberIdx}_pas_foto`,
                member.pasFotoFile
              );

            if (member.kartuPelajarFile)
              formData.append(
                `t${teamIdx}_m${memberIdx}_kartu_pelajar`,
                member.kartuPelajarFile
              );
        ...
        <div className="mb-4 text-center">
          ...
          <div className="flex flex-col items-center gap-2">
            <p className="text-teal-900 font-bold bg-white/30 backdrop-blur-sm px-6 py-1 rounded-full inline-block uppercase tracking-wider text-sm">
              {step === 0
                ? "PILIH PAKET"
                : `TIM ${currentTeamIndex + 1} - STEP ${step} OF 4`}
            </p>
          </div>
        </div>
          ...
          {step === 0 && (
            <div className="grid md:grid-cols-2 gap-6 py-10 animate-in fade-in duration-500">
              ...
              >
                ...
                <h3 className="text-2xl font-black text-teal-900 uppercase">
                  Paket Bundle
                </h3>
                <p className="text-teal-800">Daftar untuk 3 Tim Sekaligus</p>
                <span className="text-xs font-bold text-teal-900 bg-white/40 px-2 py-1 rounded uppercase">
                  Lebih Hemat
                </span>
              </button>
            </div>
          ...
          <form onSubmit={handleProcessNext} className="space-y-8">
            {step === 1 && (
              <div className="grid md:grid-cols-2 gap-8 animate-in fade-in duration-500">
                <div className="md:col-span-2 border-b border-teal-900/10 pb-2">
                  <h2 className="text-2xl font-black text-teal-900 uppercase">
                    Data Kelompok {regType === "bundle" && currentTeamIndex + 1}
                  </h2>
                </div>
                <div className="space-y-2">
                  <Label className="text-teal-900 font-bold ml-1">
                    Nama Kelompok
                  </Label>
                  <Input
                    name="namaKelompok"
                    ...
                    value={
                      allTeamsData[currentTeamIndex].groupData.namaKelompok
                    }
                ...
                {allTeamsData[currentTeamIndex].members.map((m, i) => (
                  ...
                  >
                    <div className="flex items-center gap-4 border-b border-teal-800/10 pb-4">
                      ...
                      <h3 className="text-teal-900 font-black text-2xl uppercase tracking-tight">
                        Data Anggota {i + 1}
                      </h3>
                    </div>
                    ...
                    <div className="grid md:grid-cols-2 gap-x-8 gap-y-6">
                      ...
                      <div className="space-y-2">
                        ...
                        <p className="text-black text-xs ml-1">
                          seragam sekolah/kemeja rapi, background polos
                        </p>
                        <Input
                          type="file"
                          accept="image/*"
                          onChange={(e) =>
                            handleMemberFileChange(i, "pasFotoFile", e)
                          }
                        ...
                        {m.pasFotoFile && (
                          <p className="text-xs text-teal-800">
                            File terpilih: {m.pasFotoFile.name}
                          </p>
                        )}
                      </div>
                      ...
                      <div className="space-y-2">
                        ...
                        <p className="text-black text-xs ml-1">
                          lampirkan scan kartu pelajar
                        </p>
                        <Input
                          type="file"
                          accept="image/*"
                          onChange={(e) =>
                            handleMemberFileChange(i, "kartuPelajarFile", e)
                          }
                          className={fileInputClass}
                        ...
                        {m.kartuPelajarFile && (
                          <p className="text-xs text-teal-800">
                            File terpilih: {m.kartuPelajarFile.name}
                          </p>
                        )}
                      </div>
                      ...
                      <div className="space-y-2">
                        <Label className="text-teal-900 font-bold ml-1 uppercase text-xs tracking-widest">
                          bukti Follow @ceg.ubaya
                        </Label>
                        <Input
                          type="file"
                          accept="image/*"
                          onChange={(e) =>
                            handleMemberFileChange(i, "followCegFile", e)
                          }
                          className={fileInputClass}
                        ...
                        {m.followCegFile && (
                          <p className="text-xs text-teal-800">
                            File terpilih: {m.followCegFile.name}
                          </p>
                        )}
                      </div>
                      ...
                      <div className="space-y-2">
                        <Label className="text-teal-900 font-bold ml-1 uppercase text-xs tracking-widest">
                          bukti Follow @officialtkubaya
                        </Label>
                        <Input
                          type="file"
                          accept="image/*"
                          onChange={(e) =>
                            handleMemberFileChange(i, "followTkFile", e)
                          }
                          className={fileInputClass}
                        ...
                        {m.followTkFile && (
                          <p className="text-xs text-teal-800">
                            File terpilih: {m.followTkFile.name}
                          </p>
                        )}
                      </div>
                    </div>
              ...
              <div className="p-6 md:p-8 rounded-[40px] bg-teal-900 text-white space-y-6 shadow-2xl animate-in zoom-in duration-300 border border-white/10">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-white/10 p-6 rounded-3xl backdrop-blur-sm border border-white/5">
                  <div className="space-y-1">
                    <p className="text-xs opacity-70 italic font-medium uppercase tracking-wider">
                      Transfer Ke (BCA):
                    </p>
                  ...
                  <div className="md:text-right pt-4 md:pt-0 border-t md:border-t-0 border-white/10">
                    <p className="text-xs opacity-70 uppercase font-medium tracking-wider">
                      Total Bayar:
                    </p>
                    <p className="text-3xl md:text-5xl font-black text-yellow-400 mt-1">
                      {formattedHarga}
                    </p>
                  </div>
                </div>
                ...
                <div className="space-y-3 pt-2">
                  <Label className="font-bold text-lg ml-1 block">
                  ...
                  {currentBuktiFile && (
                    <p className="text-xs text-yellow-400">
                      File terpilih: {currentBuktiFile.name}
                    </p>
                  )}
                </div>
              </div>
            ...
            {step === 4 && (
              <div className="max-w-2xl mx-auto space-y-6 text-center animate-in zoom-in duration-300">
                ...
                <h2 className="text-3xl font-black text-teal-900 uppercase">
                  Data Terverifikasi
                </h2>
                <p className="text-teal-800 font-medium italic">
                  Silakan klik "DAFTAR SEKARANG" untuk menyelesaikan
                  pendaftaran.
                </p>
              </div>
            )}
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-teal-900/60 backdrop-blur-sm p-4">
          <div className="bg-white rounded-3xl p-8 max-w-sm w-full text-center space-y-6 shadow-2xl">
            ...
            <p className="text-teal-800">
              Data pendaftaran sudah kami terima. Sampai jumpa di CEG 2026!
            </p>
            <Button
